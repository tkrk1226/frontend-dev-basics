...
[객체]
 - object
    1. new 생성자 함수() 사용해서 생성
        ...
        Object() => object type
        Array() => object type
    2. {} 객체 리터럴, JSON 사용 - J(ava)S(cript) O(bject) N(otation)
    3. [] 배열 리터럴
    4. null
 - function
    1. function f() {...} 생성
    2. var f = function() {...}
    3. var f = new Function("param1", "param2", ... ,"return ...");
*/
...
// 원시 타입에 메서드가 호출될 때, 
// 임시 Wrapper 객체가 만들어져 메서드 호출이 일어난 뒤 사라진다. (유사 객체)
console.log("=== 원시타입도 메서드 호출이 가능하다? =====================");
console.log(b.valueOf()); // JS Engine 안에서 new Boolean(b).valueOf(); 유사 객체로 쓰이고 사라진다.
